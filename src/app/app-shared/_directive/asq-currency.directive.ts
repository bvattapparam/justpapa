...
@Directive({
  selector: '[appAsqCurrency]'
})
export class AsqCurrencyDirective {
  private el: any;
  constructor(
    private elementRef:ElementRef,
    private formatcurrencypipe:AsqCurrencyPipe
  ) {
  ...
  onFocus(value,event) {
    this.el.value = this.formatcurrencypipe.transform(value); // opossite of transform
    if(event.which== 9)
    {
        return false;
    }
